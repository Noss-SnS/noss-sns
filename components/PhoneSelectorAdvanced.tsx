...
interface PhoneSelectorAdvancedProps {
...
const phoneImages: Record<string, string> = {
  ...
  'Xiaomi 14 Pro': 'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=400',
  'OnePlus 12': 'https://images.unsplash.com/photo-1598327105666-5b89351aff97?w=400',
};

export function PhoneSelectorAdvanced({ onPhoneSelect, selectedPhone }: PhoneSelectorAdvancedProps) {
  ...
  const phoneCategories = [
    ...
    },
    {
      brand: "Motorola",
      models: [
        "Motorola Edge 50 Pro", "Motorola Edge 40 Pro", "Motorola Edge 40",
    ...
    },
    {
      ...
      models: [
        ...
        "Huawei Nova 12 Pro", "Huawei Nova 11 Pro", "Huawei Nova 10"
      ]
    },
    {
      brand: "Nothing",
    ...
    },
    {
      brand: "Sony",
      models: [
        "Sony Xperia 1 VI", "Sony Xperia 1 V", "Sony Xperia 5 V", "Sony Xperia 10 V",
  ...
  const getPhoneImage = (phone: string) => {
    return phoneImages[phone] || 'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=400';
  };
    ...
    <div className="space-y-4">
      <div className="relative">
        ...
        <Input
          ...
          className="pl-10 h-14 bg-slate-900/50 border-orange-500/30 text-white placeholder:text-orange-100/30 focus:border-orange-500 focus:ring-orange-500/20 text-base"
        />
      </div>

      <Select value={selectedPhone} onValueChange={onPhoneSelect}>
        <SelectTrigger className="w-full h-14 bg-slate-900/50 border-orange-500/30 text-white hover:border-orange-500/50 focus:border-orange-500 focus:ring-orange-500/20 text-base">
          ...
          {filteredCategories.length > 0 ? (
            filteredCategories.map((category) => (
              <SelectGroup key={category.brand}>
                <SelectLabel className="text-orange-500 text-base">{category.brand}</SelectLabel>
                {category.models.map((model) => (
                  <SelectItem 
                    key={model} 
                    value={model}
                    className="focus:bg-orange-500/20 focus:text-white cursor-pointer text-base py-3"
                  >
      ...
      <AnimatePresence>
        {selectedPhone && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="bg-gradient-to-br from-orange-500/10 to-red-600/10 border border-orange-500/30 rounded-xl p-6 flex items-center gap-6"
          >
            <div className="relative size-24 flex-shrink-0">
              <ImageWithFallback
                src={getPhoneImage(selectedPhone)}
                alt={selectedPhone}
            ...
            <div className="flex-1">
              <h4 className="text-white mb-1">Téléphone sélectionné</h4>
              <p className="text-orange-400 text-lg">{selectedPhone}</p>
              <p className="text-orange-100/60 text-sm mt-2">
                Paramètres optimisés disponibles ✓
              </p>
            </div>
          </motion.div>
        )}
